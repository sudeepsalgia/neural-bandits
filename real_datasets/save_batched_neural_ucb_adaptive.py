...
for batch_param in [5, 10]:
	...
	settings = {'T': time_horizon,
				...
				'delta': delta,
				'algo': 'BatchedNeuralUCB_adaptive',
				'eta': eta,
				'lambda': _lambda }                                        
	...
	for n in range(n_sim):
		...
		model = BatchedNeuralUCB(bandit, hidden_size=hidden_size, _lambda=_lambda, delta=delta, nu=nu, training_window=time_horizon, batch_type='adaptive',
	...
	filename = './' + settings['algo'] + '_' + settings['reward_func'] + '_' + str(int(settings['batch_param'])) + '_2000_s2.pkl'
